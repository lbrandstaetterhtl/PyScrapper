...
async def handle_create_downloaded_media_req(key: str, req: CreateDownloadedMediaRequest):
    try:
        ...
        cursor.execute("""INSERT INTO DownloadedMedias (Identifier, UserIdentifier, Url, MediaType, DownloadedAt, DownloadPath, IsPlayable, Title) VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", (identifier, user_identifier, url, mediatype, downloaded_at, download_path, is_playable, titel))
        conn.commit()
        conn.close()
...
@app.get("/getuser/downloadedmedias/{key}")
async def get_user_medias(key: str, user_identifier: str):
    if key != ADMIN_KEY:
        raise fastapi.HTTPException(status_code=401, detail="Unauthorized")
